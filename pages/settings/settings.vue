  ...
  <view class="settings-page">
    <scroll-view scroll-y class="settings-scroll-view">
      ...
      <view v-if="userStore.isLoggedIn" class="setting-group">
        <view class="setting-item" @click="navigateTo('/settingPages/notifications/notifications')">
      ...
      <view v-if="userStore.isLoggedIn" class="setting-group">
        <view class="setting-item" @click="showLogoutNotice">
          ...
          <view class="item-right">
            <text class="item-value warning">注销后无法恢复</text>
            <wd-icon name="arrow-right" size="16px" color="#ccc"></wd-icon>
          </view>
        </view>
      </view>
    ...
    <bottom-popup v-model:visible="showNoticePopup" body-height="60vh" title="注销账号须知">
    ...
    <!-- 退出登录确认弹窗 -->
    <action-confirmation-modal v-model:show="showLogoutConfirm" @confirm="confirmLogout">
